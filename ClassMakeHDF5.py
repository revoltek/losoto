...
class ClassMakeHDF5():
    ...
    def load_gds(self, gdsfiles, clusterdesc, globaldb='globaldb', sky_name='sky', instrument_name= 'instrument', stations=[], sources=[] ):
        ...
        self.phases = self.hdf5.createCArray(self.hdf5.root, 'phases', tables.Float32Atom(), 
                                             #shape=(self.N_times, self.N_freqs, self.N_stations, self.N_sources, self.N_pol), 
                                             shape=(self.N_times, self.N_freqs, self.N_stations, self.N_sources, 2,2), 
                                             chunkshape = chunkshape)
        self.amplitudes = self.hdf5.createCArray(self.hdf5.root, 'amplitudes', tables.Float32Atom(), 
                                                 #shape=(self.N_times, self.N_freqs, self.N_stations, self.N_sources, self.N_pol), 
                                                 shape=(self.N_times, self.N_freqs, self.N_stations, self.N_sources, 2,2), 
                                                 chunkshape = chunkshape)
        ...
        for gdsfile, instrumentdb_name, gdsfile_idx in zip(gdsfiles, self.instrumentdb_name_list, range(len(gdsfiles))) :
            ...
            for pols in [(i,j) for i in range(2) for j in range(2)]:
                ...
                for station, station_idx in zip(self.stations, range(len(self.stations))):
                    if self.GainEnable:
                        parmname0 = ':'.join(['Gain', str(pol0), str(pol1), infix[0], station])
                    parmname1 = ':'.join(['Gain', str(pol0), str(pol1), infix[1], station])
                    print "parameter:",parmname0,parmname1
                    if self.PhasorsEnable:
                        gain_phase = instrumentdb.getValuesGrid( parmname1 )[ parmname1 ]['values']
                        self.phases[:, sorted_freq_selection, station_idx, :, pol0,pol1] = resize(gain_phase, (self.N_sources, N_freqs, self.N_times)).T
                        try:
                            gain_amplitude = instrumentdb.getValuesGrid( parmname0 )[ parmname0 ]['values']
                        except KeyError:
                            self.amplitudes[:, sorted_freq_selection, station_idx, :, pol0,pol1] = numpy.ones((self.N_times, N_freqs, self.N_sources))
                        else:
                            self.amplitudes[:, sorted_freq_selection, station_idx, :, pol0,pol1] = numpy.resize(gain_amplitudes, (self.N_sources, N_freqs, self.N_times)).T
                    else:
                        gain_real = instrumentdb.getValuesGrid( parmname0 )[ parmname0 ]
                        gain_imag = instrumentdb.getValuesGrid( parmname1 )[ parmname1 ]
                        self.phases[:, sorted_freq_selection, station_idx, :, pol0,pol1] = numpy.resize(numpy.arctan2(gain_imag['values'], gain_real['values']),
                                                                                                      (self.N_sources, N_freqs, self.N_times)).T
                        self.amplitudes[:, sorted_freq_selection, station_idx, :, pol0,pol1] = numpy.resize(numpy.sqrt(gain_imag['values']**2 + 
                                                                                                                    gain_real['values']**2),(self.N_sources, N_freqs, self.N_times)).T
                if self.DirectionalGainEnable:
                    for source, source_idx in zip(self.sources, range(len(self.sources))) :
                        parmname0 = ':'.join(['DirectionalGain', str(pol0), str(pol1), infix[0], station, source])
                        parmname1 = ':'.join(['DirectionalGain', str(pol0), str(pol1), infix[1], station, source])
                        if self.PhasorsEnable:
                            gain_phase = instrumentdb.getValuesGrid( parmname1 )[ parmname1 ]['values']
                            self.phases[:, sorted_freq_selection, station_idx, source_idx, pol0,pol1] += gain_phase
                            try:
                                gain_amplitude = instrumentdb.getValuesGrid( parmname0 )[ parmname0 ]['values']
                            except KeyError:
                                pass
                            else:
                                self.amplitudes[:, sorted_freq_selection, station_idx, source_idx, pol0,pol1] *= gain_amplitude
                        else:
                            ...
                            self.phases[0:l, sorted_freq_selection, station_idx, source_idx, pol0,pol1]  += numpy.arctan2(gain_imag, gain_real)
                            self.amplitudes[0:l, sorted_freq_selection, station_idx, source_idx, pol0,pol1] *= numpy.sqrt(gain_real**2 + gain_imag**2)
            freq_idx += N_freqs
