...
def plot_frame(screen, fitted_phase1, residuals, weights, x, y, k, lower,
    ...
    real_diff = np.abs(np.cos(fitted_phase1[nonflagged]) - np.cos(fitted_phase1[nonflagged]-residuals[nonflagged]))
    imag_diff = np.abs(np.sin(fitted_phase1[nonflagged]) - np.sin(fitted_phase1[nonflagged]-residuals[nonflagged]))
    phase_diff = np.arctan2(real_diff, imag_diff)
    nsrcs = residuals[nonflagged].shape[0]
    redchi2 = np.sum(np.square(np.abs(phase_diff)) * weights[nonflagged]) / (nsrcs-order)
    if sindx >= 0:
